...
const BaileyCalendarThemes = () => {
  ...
  console.log('🎯 BaileyCalendarThemes render state:', { 
    selectedTheme, 
    isLoading, 
  ...
  const sendDayWebhook = async (day: string) => {
    ...
    console.log(`📥 FULL webhook response for Bailey:`, responseData);
    ...
    console.log('🎯 Final mapped ideas count for Bailey:', mappedIdeas.length);
    return mappedIdeas;
  };

  const generateThemedIdeas = async (theme: string, day: string) => {
    console.log('🎯 generateThemedIdeas called for Bailey', { theme, day });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-teal-100 via-teal-50 to-cyan-50 relative overflow-hidden">
      {/* Header */}
      <header className="relative z-20 w-full py-4 bg-white/80 backdrop-blur-sm border-b border-teal-100">
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between">
            <div className="flex-1">
              <Button
                onClick={() => navigate('/bailey-dashboard')}
                variant="outline"
            ...
            <div className="flex-1 flex items-center justify-end">
              ...
              <span className="text-gray-700 font-medium">Bailey</span>
            </div>
          </div>
        </div>
      </header>
      ...
      <div className="absolute inset-0">
        ...
        <div className="absolute bottom-40 right-32 w-48 h-48 bg-teal-400/15 rounded-full blur-3xl"></div>
        <div className="absolute top-1/3 right-20 w-24 h-24 bg-cyan-300/25 rounded-full blur-xl"></div>
      </div>
              ...
              return (
                <Card 
                  ...
                  className={`group cursor-pointer hover:shadow-2xl transition-all duration-300 hover:scale-105 backdrop-blur-sm min-h-[400px] ${
                    isToday 
                      ? 'bg-gradient-to-br from-teal-200 via-teal-100 to-cyan-100 border-teal-300 shadow-xl ring-2 ring-teal-400/50' 
                      : 'bg-white/95 border-gray-200 hover:border-teal-300'
                  } rounded-2xl`}
                  onClick={() => {
                ...
                >
                  ...
                  <CardContent className="px-6 pb-6">
                    ...
                    {(themeDay.day === 'Friday' || themeDay.day === 'Saturday') ? (
                      ...
                      <Button 
                        className="w-full py-3 px-6 bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white border-0 rounded-xl font-semibold text-base shadow-lg hover:shadow-xl transition-all duration-300"
                        onClick={(e) => {
                          e.stopPropagation();
          ...
          <div className="max-w-6xl mx-auto">
            ...
            {isLoading ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm text-teal-600 mt-2">This may take up to 5 minutes, please be patient!</p>
              </div>
            ) : ideas.length > 0 ? (
                        ...
                        <div key={type} className="space-y-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {typeIdeas.map((idea) => (
                              <Card
                                key={idea.id}
                                className="group hover:shadow-lg transition-all duration-300 cursor-pointer border-2 hover:border-teal-300"
...
export default BaileyCalendarThemes;
