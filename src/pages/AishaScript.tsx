...
const AishaScript = () => {
  ...
  useEffect(() => {
    const selectedIdea = localStorage.getItem('selectedIdeaAisha');
    if (selectedIdea) {
      setTopic(selectedIdea);
      localStorage.removeItem('selectedIdeaAisha');
    }
  }, []);
  ...
  const generateScript = async () => {
    ...
    try {
      ...
      console.log("Aisha script webhook response:", data);
      
      if (data.output && typeof data.output === 'object') {
        setScriptData(data.output);
        toast({
          title: "Scripts Generated!",
          description: `Aisha created ${data.output.scripts?.length || 0} script variations for you.`,
        });
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-100 via-purple-50 to-indigo-50">
      {/* Header */}
      <header className="relative z-20 w-full py-3 sm:py-4 bg-white/80 backdrop-blur-sm border-b border-purple-100">
      ...
      <div className="absolute inset-0">
        ...
        <Sparkles className="absolute top-32 right-1/4 w-8 h-8 text-purple-400/40" />
        <Sparkles className="absolute bottom-1/3 left-1/4 w-6 h-6 text-indigo-400/50" />
      </div>

      <div className="relative z-10 container mx-auto px-4 py-6 sm:py-12">
        <div className="text-center mb-8 sm:mb-12">
          <div className="flex flex-col sm:flex-row items-center justify-center mb-4 sm:mb-6">
            <div className="bg-purple-500 p-2 sm:p-3 rounded-2xl mb-3 sm:mb-0 sm:mr-4">
              <FileText className="w-6 h-6 sm:w-8 sm:h-8 text-white" />
            </div>
            <h1 className="text-2xl sm:text-4xl md:text-6xl font-bold text-gray-900 text-center">
              Aisha's Script Generator
            </h1>
          </div>
          <p className="text-base sm:text-xl text-gray-700 mb-6 sm:mb-8 max-w-2xl mx-auto px-4">
            Transform your ideas into compelling video scripts with Aisha's AI assistance
          </p>
        </div>

        <div className="max-w-2xl mx-auto mb-8 sm:mb-12">
          <Card className="bg-white/80 backdrop-blur-sm border-purple-100 shadow-lg mx-4 sm:mx-0">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    className="mt-2 bg-white/50 border-purple-200 text-gray-900 placeholder:text-gray-500 text-base sm:text-lg p-3 sm:p-4"
                  />
                </div>
                <Button
                ...
                >
                  {isLoading ? (
                    <>
                      ...
                      Aisha is generating scripts...
                    </>
                  ) : (
                    <>
                      <FileText className="w-5 h-5 sm:w-6 sm:h-6 mr-2" />
                      Generate Scripts with Aisha
                    </>
                  )}
                </Button>
              </div>
                  ...
                  <CardHeader className="pb-4">
                    <CardTitle className="text-gray-900 text-lg sm:text-xl flex items-center justify-between">
                      <div className="flex items-center flex-1 min-w-0">
                        <FileText className="w-5 h-5 sm:w-6 sm:h-6 mr-2 text-purple-500 flex-shrink-0" />
                        <span className="line-clamp-2">{script.label}</span>
                      </div>
                      {selectedScript?.label === script.label && (
                        <Badge className="bg-purple-500 text-white ml-2 flex-shrink-0">Selected</Badge>
                      )}
                    </CardTitle>
                  </CardHeader>
                  <CardContent className="p-6 pt-0 flex flex-col h-full">
                    ...
                    <div className="flex flex-col gap-3 mt-auto">
                      <Button
                        ...
                        size="lg"
                        className="w-full bg-purple-500 hover:bg-purple-600 text-white border-0 rounded-full py-4 text-base font-semibold shadow-md hover:shadow-lg transition-all"
                      >
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="lg"
                        className="w-full border-2 border-purple-500 text-purple-600 hover:bg-purple-50 rounded-full py-4 text-base font-semibold transition-all"
                      >
        ...
        {selectedScript && scriptData && (
          <div className="max-w-6xl mx-auto mt-12 px-4 sm:px-0 animate-fade-in">
            <div className="text-center bg-white/80 backdrop-blur-sm rounded-3xl p-8 shadow-lg border border-purple-100">
              ...
              <Button
                ...
                size="lg"
                className="bg-gradient-to-r from-purple-500 to-indigo-500 hover:from-purple-600 hover:to-indigo-600 text-white px-16 py-7 text-xl rounded-full shadow-xl hover:shadow-2xl transition-all duration-300 border-0 hover:scale-105"
              >
        ...
        <Dialog open={editingScriptIndex !== null} onOpenChange={() => setEditingScriptIndex(null)}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="text-2xl text-purple-600">Edit Script</DialogTitle>
              <DialogDescription>
              ...
              <div className="space-y-4 mt-4">
                ...
                <div className="flex items-center justify-center gap-4 py-2">
                  <Button
                    ...
                    className={!isManualEdit ? "bg-purple-500 hover:bg-purple-600" : ""}
                  >
        ...
        {!scriptData && !isLoading && (
          <div className="text-center text-gray-500">
            ...
            <p className="text-xl">Enter your topic above to generate compelling scripts with Aisha!</p>
          </div>
        )}
      </div>
    </div>
